...
import org.example.pushMatrix.constants.XxlJobConstant;
...
import org.example.pushMatrix.eunms.*;
import org.example.pushMatrix.service.CronTaskService;
...
public class XxlJobUtils {
    ...
    public XxlJobInfo buildXxlJobInfo(MessageTemplate messageTemplate) {
        ...
        if (messageTemplate.getExpectPushTime().equals(String.valueOf(PushMatrixConstant.FALSE))) {
            scheduleConf = DateUtil.format(DateUtil.offsetSecond(new Date(), XxlJobConstant.DELAY_TIME), PushMatrixConstant.CRON_FORMAT);
        }
        ...
        XxlJobInfo xxlJobInfo = XxlJobInfo.builder()
                ...
                .triggerStatus(PushMatrixConstant.FALSE)
    ...
    private Integer queryJobGroupId() {
        ...
        if (basicResultVO.getData() == null) {
            XxlJobGroup xxlJobGroup = XxlJobGroup.builder().appname(appName).title(jobHandlerName).addressType(PushMatrixConstant.FALSE).build();
