...
public class SendAssembleAction implements BusinessProcess<SendTaskModel> {
    ...
    private static ContentModel getContentModelValue(MessageTemplate messageTemplate, MessageParam messageParam) {
        ...
        /*这里通过hutool包的ReflectUtil工具类去做了反射异常校验，
       通过标准java库需要处理一些繁琐的 try-catch 块来确保代码的健壮性和异常处理
       字段类型转换等*/
    ...
    public void process(ProcessContext<SendTaskModel> context) {
        ...
        try {
            //通过任务模板id得到任务模板，任务模板为空或已被软删除，则中断责任链
            Optional<MessageTemplate> messageTemplate = messageTemplateDao.findById(messageTemplateId);
            if (!messageTemplate.isPresent() || messageTemplate.get().getIsDeleted().equals(FunctionConstant.TRUE)) {
